...
def vendeurList(request):
    vendeurs = Vendeur.objects.all().order_by('-created_at')
    context = {'vendeurs': vendeurs}
    return render(request, 'dashboard/vendeurs/listVendeur.html', context)
...
def addVendeur(request):
    ...
    if request.method == 'POST':
        ...
        wave_api_key = request.POST['wave_api_key']
        wave_api_secret_key = request.POST['wave_api_secret_key']
        # avatar = request.POST['avatar']
        intech_api_key = request.POST['intech_api_key']
        stripe_endpoint_secret = request.POST['stripe_endpoint_secret']
        stripe_publishable_key = request.POST['stripe_publishable_key']
        ...
        try:
            ...
            subject, to = f"Bienvenue 🎉 sur EASY MARKET",email
            template_src = 'mail_notification.html'
            contenu = f"Félicitations! Votre compte vient d'être créé  sur EASY MARKET."
            context = {
                'settings': settings,
                "user":vendeur,
                "contenu":contenu,
                "id":"true",
                "APP_NAMES":"EASY MARKET",
                "password":password_,
                "sujet":subject,
                "year":timezone.now().year
                }
            notify.send_email("EASY MARKET",subject, to, template_src, context)
